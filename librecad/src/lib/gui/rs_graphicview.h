...
private:
	// 添加相机图片相关成员
	QImage m_cameraImage;               // 用于保存相机拍摄的图像
	bool m_hasCameraImage = false;      // 标记是否存在相机图像
	RS_Image* m_backgroundImageEntity = nullptr; // 用于显示相机图像的RS_Image实体
	QString m_cameraImagePath;          // 用于保存相机图像文件路径
	
public:
	void setCameraImage(const QImage& image,int x, int y,int w,int h, int ImageMagnification, int ImageWidth, int ImageHeight);
	void setCameraImage2(const QImage& image, int x, int y, int w, int h, int ImageMagnification, int ImageWidth, int ImageHeight);
	void drawCameraCalibrationImage(RS_Painter* painter);
public:
