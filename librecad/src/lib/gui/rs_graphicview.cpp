...
#include<climits>      // 整数极限常量
#include<cmath>        // 数学函数

#include <QApplication>      // Qt 应用程序
#include <QDesktopWidget>    // 桌面窗口相关
#include <QAction>          // Qt 动作
#include <QMouseEvent>      // 鼠标事件
#include <QtAlgorithms>     // Qt 算法
#include <QDir>            // 目录操作
#include "rs_graphicview.h" // 当前类头文件

#include "rs_line.h"           // 直线实体
#include "rs_linetypepattern.h" // 线型模式
#include "rs_eventhandler.h"   // 事件处理器
#include "rs_graphic.h"        // 图形容器
#include "rs_grid.h"           // 网格系统
#include "rs_painter.h"        // 绘图抽象接口
#include "rs_mtext.h"          // 多行文本
#include "rs_text.h"           // 单行文本
#include "rs_settings.h"       // 设置系统
#include "rs_dialogfactory.h"  // 对话框工厂
#include "rs_layer.h"          // 图层系统
#include "rs_math.h"           // 数学工具
#include "rs_debug.h"          // 调试系统
#include "rs_color.h"          // 颜色系统
#include "rs_painterqt.h"      // Qt 绘图适配器
#ifdef EMU_C99
#include "emu_c99.h"
#endif
...
RS_GraphicView::RS_GraphicView(QWidget* parent, Qt::WindowFlags f)
	:QWidget(parent, f)
	, eventHandler{ new RS_EventHandler{this} }    // 事件处理器
	, gridColor(Qt::gray)                          // 网格颜色
	, metaGridColor{ 64, 64, 64 }                  // 主网格颜色
	, grid{ new RS_Grid{this} }                    // 网格对象
	, drawingMode(RS2::ModeFull)                   // 绘制模式
	, savedViews(16)                               // 保存的视图栈（容量16）
	, previousViewTime(QDateTime::currentDateTime())  // 上次视图更新时间
	, panning(false)                               // 平移标志
{
	...
	//setSelectedColor(QColor(128,0,128));
...
{
    qDeleteAll(overlayEntities);// 清理覆盖层实体
}
...
void RS_GraphicView::setContainer(RS_EntityContainer* container) {
	...
	// adjustOffsetControls(); // 可能需要调整偏移控件
}
...
void RS_GraphicView::drawLayer1(RS_Painter* painter) {
	...
	// 非打印预览模式：仅填充 的背景区域
	const double bgSize = 65000.0; // 逻辑单位背景尺寸
	RS_Vector bgMin(0, 0);         // 背景左下角（逻辑坐标）
	RS_Vector bgMax(bgSize, bgSize); // 背景右上角（逻辑坐标）

	// 转换为屏幕坐标
	RS_Vector screenMin = toGui(bgMin);
	RS_Vector screenMax = toGui(bgMax);

	// 计算屏幕尺寸
	int screenW = (int)(screenMax.x - screenMin.x);
	int screenH = (int)(screenMax.y - screenMin.y);
	//// 填充背景（灰色）
	//painter->fillRect(
	//	(int)screenMin.x, (int)screenMin.y,
	//	screenW, screenH,
	//	RS_Color(120, 120, 120)
	//);
	// === 新增：绘制您的相机标定图片 ===
	drawCameraCalibrationImage(painter);
	// 保存当前画笔设置
	RS_Pen originalPen = painter->getPen();

	// 设置蓝色画笔（2像素宽，实线）
	RS_Pen bluePen(RS_Color(50, 50, 255), RS2::Width02, RS2::SolidLine);
	painter->setPen(bluePen);

	double inset = 2000.0;  // 逻辑单位缩进量
	// 计算缩进后的逻辑边界
	RS_Vector insetMinLogic(bgMin.x + inset, bgMin.y + inset);
	RS_Vector insetMaxLogic(bgMax.x - inset, bgMax.y - inset);
	// 转换为屏幕坐标
	RS_Vector insetMinScreen = toGui(insetMinLogic);
	RS_Vector insetMaxScreen = toGui(insetMaxLogic);
	// 绘制缩进后的矩形
	painter->drawLine(insetMinScreen, RS_Vector(insetMaxScreen.x, insetMinScreen.y));
	painter->drawLine(RS_Vector(insetMaxScreen.x, insetMinScreen.y), insetMaxScreen);
	painter->drawLine(insetMaxScreen, RS_Vector(insetMinScreen.x, insetMaxScreen.y));
	painter->drawLine(RS_Vector(insetMinScreen.x, insetMaxScreen.y), insetMinScreen);
	// 2. 绘制红色十字中心线,虚线
	RS_Pen redPen(RS_Color(255, 50, 50), RS2::Width01, RS2::DashLineX2);
	painter->setPen(redPen);

	// 计算中心点坐标
	double centerX = (screenMin.x + screenMax.x) / 2.0;
	double centerY = (screenMin.y + screenMax.y) / 2.0;

	// 绘制水平中心线（从左到右）
	painter->drawLine(
		RS_Vector(screenMin.x, centerY),    // 左边界中点
		RS_Vector(screenMax.x, centerY)     // 右边界中点
	);

	// 绘制垂直中心线（从上到下）
	painter->drawLine(
		RS_Vector(centerX, screenMin.y),    // 下边界中点
		RS_Vector(centerX, screenMax.y)     // 上边界中点
	);
	...
	if (qApp->desktop()->logicalDpiX() > 96) {
		RS_Pen pen = penSaved;
...
void RS_GraphicView::setCameraImage2(const QImage& image, int x, int y, int w, int h, int ImageMagnification, int ImageWidth, int ImageHeight) {
	if (image.isNull()) {
		qDebug() << "相机图像为空，无法处理";
		return;  // 如果图像为空，直接返回
	}
	...
	// 清除旧的背景图像实体
	if (m_backgroundImageEntity) {
		delete m_backgroundImageEntity;
		m_backgroundImageEntity = nullptr;
	}

	// 直接使用内存中的图像，不再保存到文件
	m_cameraImage = image;
	m_hasCameraImage = true;  // 标记图像已经设置

	// 创建新的背景图像实体
	RS_ImageData imageData;
	imageData.file = "";  // 不需要文件路径
	
	imageData.uVector = RS_Vector(ImageMagnification, 0.0);
	imageData.vVector = RS_Vector(0.0, ImageMagnification);
	// 获取图像的宽度和高度
	int imageWidth = m_cameraImage.width();
	int imageHeight = m_cameraImage.height();
	

	int x1 = x * ImageMagnification;
	int y1 = y * ImageMagnification;
	int w1 = w * ImageMagnification;
	int h1 = h * ImageMagnification;
	y1 = y1 + h1;
	x1 = x1 - (-32500+ ImageWidth * ImageMagnification/2);
	y1 = -y1 + (32500+ ImageHeight * ImageMagnification / 2);
	
	imageData.insertionPoint = RS_Vector(x1,y1);
	// 使用内存中的图像数据来创建背景图像实体
	m_backgroundImageEntity = new RS_Image(nullptr, imageData);

	// 设置背景图像的内存映像，不再使用文件路径
	m_backgroundImageEntity->setImage(m_cameraImage);  // 将QImage直接传递给背景实体

	// 触发重绘，确保新图像显示
	redraw();
}
void RS_GraphicView::drawCameraCalibrationImage(RS_Painter* painter) {
	//if (!painter || !m_hasCameraImage || m_cameraImagePath.isEmpty()) return;
	if (!painter || !m_hasCameraImage ) return;
	//// 加载保存的图像文件
	//QImage img(m_cameraImagePath);
	//if (img.isNull()) {
	//	qDebug() << "无法加载相机图像：" << m_cameraImagePath;
	//	return;  // 如果图像为空，直接返回
	//}
	...
	if (m_backgroundImageEntity) {
		...
		m_backgroundImageEntity->draw(painter, this, patternOffset);
...
void RS_GraphicView::setPenForEntity(RS_Painter *painter,RS_Entity *e)
{
	...
	if (!isPrinting() && !isPrintPreview())
	{
		...
		if (e->isSelected()) {
			...
			pen.setLineType(RS2::SolidLine);  // 修改为实线
...
void RS_GraphicView::drawEntity(RS_Painter *painter, RS_Entity* e, double& patternOffset) {
	...
	if (isDraftMode()) {
        ...
        case RS2::EntityText:
			...
			drawEntityPlain(painter, e, patternOffset); // 关键修改
            break;
