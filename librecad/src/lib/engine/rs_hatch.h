...
public:
    enum RS_HatchError { HATCH_UNDEFINED = -1,
                         HATCH_OK,
                         HATCH_INVALID_CONTOUR,// 无效边界
                         HATCH_PATTERN_NOT_FOUND,// // 图案未找到
                         HATCH_TOO_SMALL,// 区域过小
                         HATCH_AREA_TOO_BIG };// 区域过大
    ...
    RS_HatchData getData() const {
		...
		double getTotalArea();// 计算填充总面积

		void calculateBorders() override;// 计算边界
