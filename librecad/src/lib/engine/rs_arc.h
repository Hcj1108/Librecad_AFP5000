...
public:
    ...
    std::vector<RS_Vector> RS_Arc::getPathPoints() const override {
       
        //segments=24 是性能和视觉效果的折中**，适合大多数场景。
        const int segments = 24;
        int scale = 100;


        std::vector<RS_Vector> points;
        double a1 = RS_Math::correctAngle(data.angle1);
        double a2 = RS_Math::correctAngle(data.angle2);
        double step2 = RS_Math::getAngleDifference(a1, a2) / segments;

        for (int i = 0; i <= segments; ++i) {
            double angle = a1 + i * step2;
            RS_Vector pt = (data.center + RS_Vector::polar(data.radius, angle))*scale;
            points.push_back(pt);
            qDebug() << "Arc point :" << pt.x << pt.y;
