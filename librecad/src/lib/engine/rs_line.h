...
public:
    ...
    //重载
    std::vector<RS_Vector> RS_Line::getPathPoints() const override{
        ...
        if (step <= 0) {
            // 如果步长无效，只返回起点和终点
            return { data.endpoint ,data.startpoint };
        ...
        // 计算步数
       int numSteps = static_cast<int>(length / step);

        // 确保至少包含起点和终点
       if (numSteps == 0) {
            return { data.startpoint, data.endpoint };
        }

        // 计算实际步长（确保均匀分布）
        double actualStep = length / numSteps;
        RS_Vector unitVector = direction * (1.0 / length);

        // 生成所有点
        points.reserve(numSteps + 1);
        for (int i = 0; i <= numSteps; ++i) {
            RS_Vector currentPoint = (data.startpoint + unitVector * (i * actualStep));
            points.push_back(currentPoint);

            if (i == 0) {
                qDebug() << "First point (start):" << currentPoint.x << currentPoint.y;
            }
            else if (i == numSteps) {
              qDebug() << "Last point (before end check):" << currentPoint.x << currentPoint.y;
            }



            else qDebug() << "Line point :" << currentPoint.x << currentPoint.y;
        }
        // 确保终点精确包含（避免浮点误差）
        if (!points.empty() && points.back().distanceTo(data.endpoint) > RS_TOLERANCE) {
            points.push_back(data.endpoint);
        }
