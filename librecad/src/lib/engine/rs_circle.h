...
public:
	std::vector<RS_Vector> RS_Circle::getPathPoints() const  {
		std::vector<RS_Vector> points;
		int numPoints = 36;
		//动态计算点数（按半径比例增加）
		if (data.radius * 0.36 > 36)
		{
			numPoints = data.radius * 0.36;
		}
		//映射到65535的范围内，放大了100倍
		...
		for (int i = 0; i < numPoints; ++i) {
			...
			RS_Vector point(
				data.center.x + data.radius * cos(angle),
				data.center.y + data.radius * sin(angle)
			);
			point = point;
			qDebug() << "Circle point :" << point.x << point.y;
		...
		qDebug() << "Circle point :" << points.front().x << points.front().y;
		return points;
	}
