...
//	gxstring strValue = "";
//	//获得图像宽度、高度等
//	m_i64ImageWidth = (int64_t)objCGXDevicePointer->GetRemoteFeatureControl()->GetIntFeature("Width")->GetValue();
//	m_i64ImageHeight = (int64_t)objCGXDevicePointer->GetRemoteFeatureControl()->GetIntFeature("Height")->GetValue();
//
//	//获取是否为彩色相机
...
// 1. 将 ANSI 字符串转换为宽字符串的函数
std::wstring ConvertAnsiToWide(const std::string& str) {
    // 计算需要的缓冲长度
    int bufferSize = MultiByteToWideChar(
        CP_ACP,             // 使用当前系统 ANSI 代码页
        MB_PRECOMPOSED,     // 预处理组合字符
        str.c_str(),        // 源 ANSI 字符串
        -1,                 // 自动计算长度
        nullptr,            // 先查询需要的长度
        0                   // 初次调用不传入缓冲区
    );
    ...
    if (bufferSize == 0) {
        return L"";  // 转换失败返回空宽字符串
    }

    // 创建目标宽字符串并分配内存
    std::wstring wstr(bufferSize - 1, L'\0');  // -1 不包含终止符
...
void CGXBitmap::SaveBmp(CImageDataPointer& objCImageDataPointer, const std::string& strFilePath)
{
    ...
    /*  
	这里使用了我自定义的一个转换函数ConvertAnsiToWide，将ANSI字符串转换为宽字符串
	并且连续调用了.c_str()，以便将其转换为LPCTSTR类型传递给CreateFile函数。
	这样可以确保在Windows API中正确处理文件路径。
                                                                   by Liang
     */
    // 转换为宽字符串
    std::wstring wideFilePath = ConvertAnsiToWide(strFilePath);
    LPCTSTR LPCSTR_ImgFileName = wideFilePath.c_str();//使用了我自定义的一个转换函数，并且连续调用了.c_str()，并对下面CreateFile进行了修改
