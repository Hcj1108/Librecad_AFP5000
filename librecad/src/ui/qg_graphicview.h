...
public:
     ...
     * @param oy, offset Y
     */
	void setOffset(int ox, int oy) override;  // 设置视图偏移
    /**
...
protected:
    ...
    QGridLayout* layout;
    //! CAD mouse cursor
    std::unique_ptr<QCursor> curCad;// CAD光标
    //! Delete mouse cursor
    std::unique_ptr<QCursor> curDel;// 删除光标
    //! Select mouse cursor
    std::unique_ptr<QCursor> curSelect;// 选择光标
	...
	std::unique_ptr<QPixmap> PixmapLayer1;  // Used for grids and absolute 0网格和绝对零点
    std::unique_ptr<QPixmap> PixmapLayer2;  // Used for the actual CAD drawing CAD图形主体
    std::unique_ptr<QPixmap> PixmapLayer3;  // Used for crosshair and actionitems十字准线和交互元素
